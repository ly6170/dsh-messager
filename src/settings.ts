 ...
 * - 命名空间 = profile 中该插件条目的**原始行 id**（本仓库 cordis.yml /
 *   cordis.patch.yml 均声明为 `messager`）；
 * - schema = 插件模块导出的 `Config`（即 src/config.ts），由 Loader 自动接管；
 * - 因此**不存在** `ctx.settings.register(ns, schema, …)` 这种 API（0.1.2 时代的
 *   写法，0.1.7 已移除，调用即崩溃）。
 *
 * 本模块只负责解析命名空间，供浏览器配置路由（src/config-route.ts）使用。
 *
 * 插件**自身的配置读取**不走这里：Loader 会把配置（volatile 引用）直接交给
 * `apply(ctx, config)` 并就地更新，用时读取即可 —— 见 src/index.ts 的 readConfig。
 */
 ...
 *
 * 解析不出来时退回条目名 `messager`，不抛错 —— 解析失败不应阻断通知功能。
 * 之所以不硬编码：不同装载方式（bundle 的 cordis.patch.yml / dev 的 cordis.yml /
 * marketplace 安装）可能给出不同的条目 id。
 *
